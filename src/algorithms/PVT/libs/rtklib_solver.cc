                    ...
                    try
                        {
                            ...
                            std::cout << "PVT lib dump enabled Log file: " << d_dump_filename.c_str();
                        }
...
{
    std::cout << "Rtklib_Solver destructor called.";
...
void Rtklib_Solver::store_has_data(const Galileo_HAS_data &new_has_data)
{
    ...
    if (new_has_data.header.orbit_correction_flag)
        {
            std::cout << "Received HAS orbit corrections";
    ...
    if (new_has_data.header.clock_fullset_flag)
        {
            std::cout << "Received HAS clock fullset corrections";
            for (const auto &gpseph : gps_ephemeris_map)
                {
                    int prn = gpseph.second.PRN;
                    int32_t sis_iod = gpseph.second.IODE_SF3;
    ...
    if (new_has_data.header.clock_subset_flag)
        {
            std::cout << "Received HAS clock subset corrections";
            for (const auto &gpseph : gps_ephemeris_map)
                {
                    int prn = gpseph.second.PRN;
                    int32_t sis_iod = gpseph.second.IODE_SF3;
    ...
    if (new_has_data.header.code_bias_flag)
        {
            std::cout << "Received HAS code bias corrections";
    ...
    if (new_has_data.header.phase_bias_flag)
        {
            std::cout << "Received HAS phase bias corrections";
            uint32_t valid_until = tmt +
    ...
    for (gnss_observables_iter = gnss_observables_map.cbegin();
         ...
         ++gnss_observables_iter)  // CHECK INCONSISTENCY when combining GLONASS + other system
        {
            switch (gnss_observables_iter->second.System)
                        ...
                        if (sig_ == "1B")
                            {
                                ...
                                if (galileo_ephemeris_iter != galileo_ephemeris_map.cend())
                                    ...
                                    {
                                        std::cout << "No ephemeris data for SV " << gnss_observables_iter->second.PRN;
                                    }
                            }
                        ...
                        if (sig_ == "1C")
                            {
                                ...
                                if (gps_ephemeris_iter != gps_ephemeris_map.cend())
                                    ...
                                    {
                                        std::cout << "No ephemeris data for SV " << gnss_observables_iter->first;
                                    }
                            }
                ...
                default:
                    std::cout << "Hybrid observables: Unknown GNSS";
                    break;
                }
        }
    ...
    if ((valid_obs + glo_valid_obs) > 3)
        {
            ...
            if (result == 0)
            ...
            else
                {
                    ...
                    std::cout << "RTKLIB Position at RX TOW = " << gnss_observables_map.cbegin()->second.RX_time
                    ...
                    std::cout << "RTKLIB Position at " << boost::posix_time::to_simple_string(p_time)
